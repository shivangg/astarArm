			 ...
			 rll_planning_project::Move::Response &resp)
{
	...
	diff_current_state(req.pose, move_dist, dist_rot);
	if ((move_dist < 0.005 && move_dist > 0.0005) || (move_dist < 0.0005 && dist_rot < 20 * M_PI / 180)) {
		ROS_WARN("move commands that cover a distance between 0 and 5 mm or sole rotations less than 20° are not supported!. move_dist=%.3f theta=%.3f", move_dist, dist_rot);
		resp.success = false;
		return true;
	}

	ROS_INFO("move request to pos x=%.3f y=%.3f theta=%.3f", req.pose.x, req.pose.y, req.pose.theta);
	pose2d_to_pose3d(req.pose, move_lin_req.pose);
	...
	if (move_lin_resp.success) {
		...
		ROS_WARN("aborting, a move command with an invalid goal position was sent. x=%.3f y=%.3f theta=%.3f", req.pose.x, req.pose.y, req.pose.theta);
			       ...
			       rll_planning_project::CheckPath::Response &resp)
{
	...
	if ((move_dist < 0.005 && move_dist != 0) || ( move_dist == 0 && dist_rot < 20 * M_PI / 180)) {
		ROS_WARN("check path requests that cover a distance between 0 and 5 mm or sole rotations less than 20° are not supported! move_dist=%.3f theta=%.3f", move_dist, dist_rot);
...
bool PlanningIface::check_goal_state()
{
	...
	diff_current_state(goal_pose_2d, dist_goal_trans, diff_goal_angle);
